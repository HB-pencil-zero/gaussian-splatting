...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image))   +   0.01 * torch.exp(gaussians._scaling).norm()     #0.05 * torch.exp(gaussians._scaling).norm()  
        loss.backward()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[2000,10_000, 30_000, 50_000, 80_000, 100_000, 150_000, 200_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[ 2000 , 10_000, 30_000, 50_000, 80_000, 100_000, 150_000, 200_000])
